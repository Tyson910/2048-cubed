...
export default class Grid {
  ...
  cells: null[] | Cube['position'][];

  constructor(dimensions: Grid['dimensions']) {
    this.dimensions = dimensions;
    this.cells = Grid.buildFlatArray(dimensions);
  }

  // build a 1D array given 3D dimensions
  static buildFlatArray(dimensions: Grid['dimensions']) {
    const [width, height, depth] = dimensions;
    return fill(Array(width * height * depth), null)
  };

  isCellAvailable = (cell: Cube['position']) => !this.isCellOccupied(cell);

  isCellOccupied = (cell: Cube['position']) => !!this.getCellContent(cell);
  ...
  getCellContent = (cell: Cube['position']) => this.isWithinBounds(cell) && this.cells[this.positionToIndex(cell)];

  findAvailableCells = () => {
    const cells: Exclude<Grid['cells'], null[]> = [];
  ...
  indexToPosition = (position: number) => {
  ...
  positionToIndex = ({ x, y, z }: Cube['position']) => {
    // TODO: Validate depth usage
    const [width, height, depth] = this.dimensions;
    return x + (y * height) + (z * width * height);
  };

  forEachCell = (cb: (x: number, y: number, z: number, cell: Cube['position'] | null) => void) => {
  ...
  isWithinBounds = (position: Cube['position']) => {
